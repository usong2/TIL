...
콜백 함수로 프로퍼티를 수정하는 방법

+ ./src/03에 CounterApp.jsx 생성
...
+ ./src/03/Counter2.jsx 수정
  ...
  코드를 작성한 후 [카운트 증가]를 눌러도 제대로 작동하지 않음

  ```bash
  Uncaught TypeError: this.setState is not a function
  ```

  콘솔 오류 메세지는 Counter2 컴포넌트에서 프로퍼티로 받아 실행한 onAdd() 함수에서 참조하는 this의 범위 때문에 발생한 것. onAdd() 함수에 구현되어 있는 this.setState(...)는 상위 컴포넌트(App)에 정의되어 있는데 하위 컴포넌트(Counter2)에서 실행되기 때문. 이렇게 콜백 함수를 프로퍼티 데이터로 전달하는 경우에는 하위 컴포넌트에서 참조할 this 범위에 대한 오류를 주의해야 함. 

### bind() 함수로 this 범위 오류 해결하기

this 범위 오류는 bind() 함수를 사용하면 해결 가능
...
+ ./src/App.js 수정
  ...
  class CounterApp extends React.Component {
    ...
    render() {
      return <Counter count={this.state.count} onAdd={this.increateCount.bind(this)} />;
    }
  }
  ...
  이제 버튼이 제대로 동작하는데 increateCount() 함수가 호출되는 this 범위가 App 컴포넌트에 묶인 것이며 이때 bind() 함수는 constructor() 함수에 모아두면 매번 render() 함수를 호출할 때마다 새로 작성하지 않아도 됨
...
컴포넌트에서 window.addEventListener() 함수와 같은 DOM 객체 함수를 사용하려면 DOM 객체를 컴포넌트 변수에 할당해야 함. 이때 특수 프로퍼티 ref를 사용하는데 ref 프로퍼티는 document.getElementById()가 반환하는 객체를 반환. 단, ref 프로퍼티는 DOM 객체 함수가 필요한 엘리먼트에 콜백 함수 형태로 전달.

화면에서 스크롤 위치를 측정해 현재 컴포넌트의 위치가 화면 안에 있는지 알려주는 컴포넌트

+ ./src/03에 ScrollSpy.jsx 생성
  ...
  export default class ScrollSpy extends React.PureComponent {
    constructor(props) {
      ...
      window.addEventListener('scroll', this.checkPosition); // 윈도우 스크롤 이동 이벤트를 감지하여 checkPosition() 함수 실행
    }
    setRef(ref) {
    ...
    componentWillUnmount() {
      window.removeEventListener('scroll', this.checkPosition); // 컴포넌트 소멸 시 화면 스크롤 감시 함수를 메모리에서 제거
    }
  ...
  위 코드는 페이스북에서 스크롤을 내릴 때 자동으로 다음 페이지 목록을 추가할 때 사용되는데 화면 스크롤이 변화할 때마다 객체 함수의 위치를 읽어 현재 위치가 화면 안팎인지 측정하고 있음

  console.log('enter')에 해당하는 부분에 다음 페이지 목록을 추가하는 코드를 구현하면 스크롤이 내려갈 때마다 자동으로 다음 페이지 목록이 추가되도록 구현 가능

### 컴포넌트에서 DOM 이벤트 사용하기

컴포넌트에서 출력된 특정 DOM 객체에 이벤트 컴포넌트가 반응하기 위해서는 DOM 이벤트 프로퍼티를 사용해야 함. 컴포넌트에 출력된 버튼이 클릭되었을 때 카운트 숫자가 증가하는 경우가 이러한 예. 

이벤트 프로퍼티는 특수 프로퍼티로 콜백 함수 형태로 전달해 처리하며 HTML 엘리먼트의 이벤트들은 JSX 내에서 'on + 이벤트명' 형태의 프로퍼티로 제공되고 있음

| 이벤트 이름 | 이벤트 호출 시점                                  | JSX DOM 이벤트 프로퍼티 |
| ----------- | ------------------------------------------------- | ----------------------- |
| click       | 엘리먼트의 마우스와 키보드가 클릭될 때            | onClick                 |
| submit      | 폼 데이터가 전송될 때                             | onSubmit                |
...
맵핑된 프로퍼티는 기존 카운터 예제와 같이 콜백 함수 형태로 이벤트 처리가 가능
...
리액트는 프로퍼티, state와 같은 데이터를 상위 컴포넌트에서 하위 컴포넌트 방향으로 전달하는데, 만약 데이터 변경이 필요한 경우 콜백 함수를 호출하여 원본 데이터가 위치한 상위 컴포넌트에서 데이터를 변경하고 다시 자식 컴포넌트로 전달하도록 만듦

이와 같은 데이터 흐름을 사용하는 기법을 '단방향 흐름 방식'이라고 하는데 단방향 흐름 방식은 원본 데이터의 무결성을 지켜주므로 데이터 수정으로 인한 데이터 파편화를 줄여줌